...
def output_as_tex_table(sequence):
    ...
    for t in sequence:
        ...
        t_copy = copy.copy(t)
        shuffle(t_copy)
        table += "      {\\Huge \\emoji[ios]{" + str(t_copy[0])  + "}} \\hspace{10mm} {\\Huge \\emoji[ios]{" + str(t_copy[1]) + "}} \\hspace{10mm} {\\Huge \\emoji[ios]{" + str(t_copy[2])  + "}} & {\\Huge \\emoji[ios]{" + str(solution) + "}} \\\\" + "\n"
...
def output_as_tex_file(sequences):
    ...
    for seq in sequences:
        ...
        document += "\\section*{Defuse Codes Page " + str(i).zfill(3) + "}" + "\n"
